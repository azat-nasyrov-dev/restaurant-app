...
describe('Review', () => {
  ...
  beforeEach(() => {
    ...
    reviewsCount = view.find('Review').length;
    name = view.find('[data-id="review-user"]').text();
    text = view.find('[data-id="review-text"]').text();
    rate = view.find('svg[data-id="full-star"]').length;
  });
...
describe('Anonymous Review', () => {
  it('should render anonymous name', () => {
    const view = render({ rating: 1 });
    expect(view.find('[data-id="review-user"]').text()).toBe('Anonymous');
  });
});
